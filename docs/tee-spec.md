...
| P8  | Bitcoin inclusion proofs valid against the in-enclave chain | OK; plus the calldata `proof` is required (fail-closed): both `(height, commitmentHash)` pairs must match the enclave's own headers, `source` is pinned to the block anchoring the consignment's last witness tx (re-verified under one lock guard), and `latest` must be within `MAX_RELAY_TIP_LAG_BLOCKS = 100` of the enclave tip (#57/#122) |
