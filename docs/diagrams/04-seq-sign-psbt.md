...
sequenceDiagram
    ...
    Srv->>Anchor: last transition == the build flow's deposit shape:<br/>rgb-swap ⇒ TS_TRANSFER, asset_output_amount ≥ amount − commission<br/>rgb-mint-burn ⇒ TS_INFLATION (or TS_BRIDGE with bfa-mint),<br/>asset_output_amount == amount − commission<br/>(OS_ASSET only; OS_INFLATION allowance excluded)
