...
[`docs/tee-spec.md`](../tee-spec.md). Refreshed 2026-09-07 (typed
`TeeFundsOut` digests, per-image RGB flows, invoice recipient bind,
Concordium, Electrum resolver).
...
| [`04-seq-sign-psbt.md`](04-seq-sign-psbt.md) | Bridge PSBT path (EVM → RGB): independent FundsIn verification, consignment anchoring with per-output legs and the invoice recipient bind, fee sanity, then taproot script-path (Schnorr) signing scoped to the colored account. Plain-BTC signing is the separate `SignBtc` request. |
