...
| [`04-seq-sign-psbt.md`](04-seq-sign-psbt.md) | Bridge PSBT path (EVM → RGB): provider-based deposit verification, consignment anchoring with per-output legs and the invoice recipient bind, fee sanity, then BIP-86 key-path taproot (Schnorr) signing scoped to the colored account. Plain-BTC signing is the separate `SignBtc` request. |
