...
[`docs/tee-spec.md`](../tee-spec.md). Refreshed 2026-07-28 (`networks/`
layout, unified `Sign` request, `SignBtc` split, attested security policy).
...
| [`04-seq-sign-psbt.md`](04-seq-sign-psbt.md) | Bridge PSBT path (EVM → RGB): independent FundsIn verification (Helios optional), consignment anchoring, fee sanity, then taproot script-path (Schnorr) + SegWit v0 P2WSH (ECDSA), both anchored to `witness_utxo.script_pubkey`. Plain-BTC signing is the separate `SignBtc` request. |
