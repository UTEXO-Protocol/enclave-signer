...
use bitcoin::{Amount, OutPoint, ScriptBuf, Sequence, Transaction, TxIn, TxOut, Txid, WScriptHash};
...
fn skips_when_witness_script_does_not_hash_to_script_pubkey() {
    let (pk, btc_pk) = ours();
    let real_ws =
        build_2of3_witness_script(&[pk_from_byte(0x02), pk_from_byte(0x03), pk_from_byte(0x04)]);
